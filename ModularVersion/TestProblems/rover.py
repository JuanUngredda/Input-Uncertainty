...
class RoverReward:
    def __init__(self):
        ...
        self.xmin = np.zeros(60)
        self.xmax = np.ones(60)
        self.dx = 60
        ...
        # maximum value of f
        f_max = 5.0
        self.objective = ConstantOffsetFn(domain, f_max)
        self.objective = NormalizedInputFn(self.objective, raw_x_range)

    def get_range(self):
        return self.objective.get_range()

    def __call__(self, argv):
        return self.objective(argv)

    def testmode(self, x, num_seeds=None):
...
if __name__ == "__main__":
    ...
    if len(args) == 1:
        ...
        if len(args) == 61:
            ...
            raise ValueError("provide 0 or 60 floats")
