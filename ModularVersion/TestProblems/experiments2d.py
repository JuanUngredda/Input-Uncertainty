...
class rosenbrock(function2d):
    ...
    def __init__(self,bounds=None,sd=None):
        ...
        self.xmin = np.array(self.bounds[0][0])
        self.xmax = np.array(self.bounds[0][1])

        # uncertainty parameter
        self.amin = np.array(self.bounds[1][0])
        self.amax = np.array(self.bounds[1][1])
    ...
    def __call__(self, x, w, *args, **kwargs):
        ...
        w = np.vstack(w)
        n = x.shape[0]
        assert x.shape == w.shape; "Different x, w vectors"
        assert x.shape[1] + w.shape[1] == self.input_dim; "wrong input dimensions"
        fval = 100*(w-x**2)**2 + (x-1)**2
        if self.sd ==0:
