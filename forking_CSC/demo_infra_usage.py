...
def run(args):
    ...
    # print something
    print("\n#################################")
    print("Running an instance of demo_infra_usage.py!")
    output = ""
    output += "\nHello world from: " + hostname
    output += "\nRunning conda env: " + conda_env
    output += "\nReceived parsed args: " + str(vars(args))
    output += "\nCurrent working dir: " + os.getcwd()
    output += "\nJob Settings: " + str(this_job_setting)
    print(output)

    # let's waste some CPU so we can see the htops light up!
    print("Doing random matmuls!")
    for i in range(200):
        x = np.random.uniform(size=(1000,1000))
        y = np.random.uniform(size=(1000,1000))
        z = np.matmul(x,y)
    print("Finished lots of random matmuls!")
